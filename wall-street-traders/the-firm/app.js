...
function renderDesks(){const agents=visibleAgents();const grid=document.querySelector("#deskGrid");grid.innerHTML=agents.map((a,i)=>`<article class="desk action-${a.action}" data-id="${a.id}" tabindex="0" aria-label="Open WST #${a.id}"><div class="station-label"><span>${a.role}</span><b class="${a.pnl>=0?"up":"down"}">${fmtPct(a.pnl)}</b></div>${i%3===0?`<span class="bubble">${a.quote}</span>`:""}<div class="portrait"><img src="${IMAGE_ROOT}${a.id}.png" alt="WST #${a.id}"></div><div class="terminal terminal-left"><i></i><small>${a.focus}</small></div><div class="terminal terminal-right"><i></i><small>${a.action.toUpperCase()}</small></div><div class="order-flash ${a.pnl>=0?"positive":"negative"}">${a.action==="buying"?"BUY":a.action==="selling"?"SELL":"LIVE"}</div><div class="counter"><b>WST #${a.id}</b><span>${a.focus} DESK</span></div></article>`).join("");grid.querySelectorAll(".desk").forEach(el=>{const open=()=>selectAgent(agentFor(records.find(r=>Number(r.tokenID)===Number(el.dataset.id))));el.addEventListener("click",open);el.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();open()}})});selectAgent(agents[0])}
